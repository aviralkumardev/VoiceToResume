...
Adds `field_completeness` as real, first-class session state (not a
debug-export side effect): a new key on every session row, and a new
lock-guarded `apply_field_completeness` method on the CRUD Protocol and its
in-memory implementation, following the exact same pattern
`apply_resume_update` already uses.

The debug JSON export (`{session_id}_status.json`) switches from
recomputing `field_status.py`'s naive presence-based signal to dumping this
real stored state directly — `field_status.py`'s import is dropped from
`crud.py` entirely (see `phase-11` for deleting the file itself, once
nothing references it).

**See also `phase-2-completeness-status.md`** for the shape of the
`field_completeness` dict this stores (one entry per `COVERAGE_SCHEMA`
block, each `{"completeness_status", "reason", "confidence"}` plus optional
`"fields"`/`"items"`).
...
**Change 1** — add `apply_field_completeness` to the Protocol, right after `apply_final_resolution`:
...
from app.meeting_room.resume_analysis_pipeline.field_status import compute_field_status
...
class InMemoryResumeRoomCRUD:
    ...
    def _write_resume_json_sync(self, session_id: str, row: Dict[str, Any]) -> None:
        ...
        field_status = compute_field_status(row["resume_data"])
        with status_path.open("w", encoding="utf-8") as f:
            json.dump(field_status, f, indent=2, ensure_ascii=False)
...
**Change 1** — drop the `field_status` import (it's no longer called anywhere in this file after Change 3):
...
(i.e. delete the `from app.meeting_room.resume_analysis_pipeline.field_status import compute_field_status` line entirely.)

**Change 2** — add `"field_completeness": {}` to the row built in `create_session`:
...
**Change 3** — `_write_resume_json_sync`'s status-file write switches from recomputing the naive signal to dumping the real stored state:
...
(replaces the two lines that called `compute_field_status(row["resume_data"])` and dumped its result.)

**Change 4** — new method, placed right after `apply_final_resolution`:
...
- **`apply_field_completeness` replaces `field_completeness` wholesale**,
  unlike `apply_resume_update`'s incremental merge — `phase-2`'s
  `merge_completeness` already produces the complete new state (carried-
  forward parts included), so there's nothing left to merge at the CRUD
  layer. This mirrors the fact that `merge_completeness` runs once per
  silence event against the *previous* `field_completeness` it was handed,
  not incrementally inside the lock.
- **No `None`-guard on `completeness_status`** the way `apply_resume_update`
  guards `updates` — the worker (`phase-9`) only ever calls this once it has
  a real merged result to store; there's no "no update" case worth a
  no-op branch here (an empty-verdict round still produces a valid, if
  unchanged, `field_completeness` dict via `merge_completeness`).
- **`_fold_llm_usage` is reused as-is**, no changes needed — it's already
  generic over "some dict of usage numbers," which is exactly what
  `run_completeness_chain`'s `_llm_usage` looks like.
- **Change 1's import removal is why `phase-11` must delete `field_status.py`
  in the same pass** — once this change lands, nothing in the codebase
  imports it any more, so leaving the file behind would be dead code with
  no caller, not a shared utility.
